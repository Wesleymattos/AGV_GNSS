...
intervalo_envio_ms = 1000  # padre3o
...
def carregar_comandos():
    ...
    if not os.path.exists(caminho):
        print("Arquivo comando.txt ne3o encontrado.")
        return []
...
def escolher_filtro_nmea():
    ...
    for i, f in enumerate(filtros_disponiveis, start=1):
        print(f"{i}: Somente {f}")
    print(f"{len(filtros_disponiveis)+1}: Sem filtro (enviar todas as sentene7as)")
    opcao = input("Escolha uma ope7e3o: ")
    try:
        idx = int(opcao)
...
def iniciar_terminal():
    ...
    if not portas:
        ...
        return
    print("Portas disponedveis:")
    for i, porta in enumerate(portas):
        print(f"{i + 1}: {porta}")
    try:
        idx1 = int(input("Escolha o nfa00mero da Porta Serial 1: ")) - 1
        idx2 = int(input("Escolha o nfa00mero da Porta Serial 2: ")) - 1
        if idx1 == idx2:
        ...
        try:
            ...
            if 0 <= idx_vel < len(velocidades_ms):
                ...
                print("Ope7e3o inve1lida. Usando padre3o de 1000 ms.")
        except:
            print("Entrada inve1lida. Usando padre3o de 1000 ms.")
