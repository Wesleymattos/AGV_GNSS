...
def escolher_filtro_nmea():
    ...
    filtros_disponiveis = ["GPGGA", "GPRMC", "GNGGA", "GPGSA", "GPGSV", "GNRMC", "GNVTG", "GNZDA", "GPVTG", "GPZDA", "GPGST"]
...
def sincronizar_envio():
    global ultima_sentenca, ultima_precisao, intervalo_envio_ms
    while True:
        ...
        db.child("gnss_precision").update({
            "rms": ultima_precisao["rms"],
            "lat_err": ultima_precisao["lat_err"],
            "lon_err": ultima_precisao["lon_err"],
            "alt_err": ultima_precisao["alt_err"]
        })
        print(f"[{timestamp}] Enviado para Firebase:")
        print(f"  raw : {ultima_sentenca['raw']}")
        print(f"  raw2: {ultima_sentenca['raw2']}")
        print(f"  Precisão -> RMS: {ultima_precisao['rms']} | Lat: {ultima_precisao['lat_err']} | Lon: {ultima_precisao['lon_err']} | Alt: {ultima_precisao['alt_err']}")
